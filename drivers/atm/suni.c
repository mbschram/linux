...
#define ADD_LIMITED(s, v) \
	do {						\
		...
		if (atomic_read(&stats->s) < 0)		\
			atomic_set(&stats->s, INT_MAX);	\
	} while (0)
...
static int fetch_stats(struct atm_dev *dev,
...
static int suni_stop(struct atm_dev *dev)
{
	...
	spin_lock_irqsave(&sunis_lock, flags);
	for (walk = &sunis; *walk != PRIV(dev); walk = &PRIV((*walk)->dev)->next)
...
int suni_init(struct atm_dev *dev)
{
	...
	dev->phy_data = kmalloc(sizeof(struct suni_priv), GFP_KERNEL);
	if (!dev->phy_data)
		return -ENOMEM;
	PRIV(dev)->dev = dev;
