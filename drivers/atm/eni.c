...
static int rx_aal5(struct atm_vcc *vcc)
{
	...
	if (descr & (MID_RED_T | MID_RED_CRC_ERR)) {
		...
		length = readl(eni_vcc->recv + (((eni_vcc->descr + size - 1) &
				...
				/* -trailer(2)+header(1) */
		if (length && length <= (size << 2) - 8 && length <= ATM_MAX_AAL5_PDU)
			eff = (length+3) >> 2;
...
static int open_rx_first(struct atm_vcc *vcc)
{
	...
	if (size > MID_MAX_BUF_SIZE && vcc->qos.rxtp.max_sdu <= MID_MAX_BUF_SIZE)
...
static enum enq_res do_tx(struct sk_buff *skb)
{
	...
	if (dma_wr != dma_rd && ((dma_rd + NR_DMA_TX-dma_wr) & (NR_DMA_TX - 1)) < dma_size) {
				...
				int set_rsv, int set_shp)
{
	...
	unlimited = ubr && (!rate || rate <= -ATM_OC3_PCR || rate >= ATM_OC3_PCR);
	if (!unlimited) {
	...
	if (!new_tx)
		...
		tx->tx_pos = eni_in(MID_TX_DESCRSTART(tx->index)) & MID_DESCR_START;
	}
...
static int eni_do_init(struct atm_dev *dev)
{
	...
	error = pci_write_config_word(pci_dev, PCI_COMMAND,
			PCI_COMMAND_MEMORY |
	...
	base = ioremap_nocache(real_base, MAP_MAX_SIZE);
	if (!base) {
		printk("\n");
...
static int eni_start(struct atm_dev *dev)
{
	...
	error = request_irq(eni_dev->irq, &eni_int, IRQF_SHARED, DEV_LABEL, dev);
	if (error) {
	...
	error = pci_write_config_word(eni_dev->pci_dev, PCI_COMMAND,
	...
	error = pci_write_config_byte(eni_dev->pci_dev, PCI_TONGA_CTRL, END_SWAP_DMA);
	if (error) {
